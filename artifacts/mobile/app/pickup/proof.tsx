  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.content, { paddingTop: Platform.OS === "web" ? 40 : 32 }]}>
        <Text style={[styles.title, { color: colors.foreground }]}>Take Pickup Photo</Text>
        <Text style={[styles.sub, { color: colors.mutedForeground }]}>
          Take a clear photo of all items with the receipt for proof of pickup
        </Text>
        <Pressable onPress={takePhoto} style={[styles.photoBox, { backgroundColor: colors.muted, borderColor: photo ? colors.success : colors.border, borderRadius: colors.radius }]}>
          {photo ? (
            ...
            <View style={styles.placeholder}>
              ...
              <Text style={[styles.tapText, { color: colors.foreground }]}>Tap to take photo</Text>
              <Text style={[styles.tapSub, { color: colors.mutedForeground }]}>Include all items + receipt</Text>
            </View>
          )}
        </Pressable>
      </View>
      <View style={[styles.footer, { paddingBottom: insets.bottom + 24 }]}>
        <Button title="Confirm Pickup" onPress={() => router.push("/pickup/complete" as any)} disabled={!photo} size="xl" />
        {photo && <Button title="Retake" onPress={takePhoto} variant="outline" />}
      </View>
    </View>
  );
}
