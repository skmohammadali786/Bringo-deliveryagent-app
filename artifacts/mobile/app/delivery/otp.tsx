...
export default function DeliveryOtpScreen() {
  ...
  const verify = () => {
    ...
    if (code !== order?.deliveryOtp) {
      setError("Incorrect OTP. Ask customer for the correct code.");
      return;
    }
    updateOrderStatus(order.id, "delivering");
    router.push("/delivery/photo-proof" as any);
  };

  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScreenHeader title="Delivery OTP" showBack />
      <View style={[styles.content, { paddingTop: Platform.OS === "web" ? 40 : 40 }]}>
        <Text style={[styles.title, { color: colors.foreground }]}>Enter Delivery OTP</Text>
        <Text style={[styles.sub, { color: colors.mutedForeground }]}>
          Ask the customer for the 4-digit OTP to confirm delivery
        </Text>
        <Text style={[styles.hint, { color: colors.mutedForeground }]}>Demo OTP: {order?.deliveryOtp}</Text>
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + 24 }]}>
        <Button title="Verify & Complete Delivery" onPress={verify} disabled={filledCount !== 4} size="xl" />
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  hint: { fontSize: 13, fontFamily: "Inter_500Medium", fontStyle: "italic" },
  ...
  digit: { fontSize: 28, fontFamily: "Inter_700Bold" },
